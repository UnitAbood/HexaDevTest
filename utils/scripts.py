...
async def get_mini_game_cipher(http_client: aiohttp.ClientSession,
                               ...
                               start_date: str,
                               game_sleep_time: int,
                               daily_mini_game_name: str):
    if settings.USE_RANDOM_MINI_GAME_KEY:
        cipher = f"0{game_sleep_time}{random.randint(10000000000, 99999999999)}"[:10]
        number = int(datetime.fromisoformat(start_date.replace("Z", "+00:00")).timestamp())
        score_per_game = {
            "Candles": 0,
            "Tiles": random.randint(int(1500), 3000),
        }
        score_cipher = 2 * (number + score_per_game[daily_mini_game_name])
        sig = base64.b64encode(hashlib.sha256(f"415t1ng{score_cipher}0ra1cum5h0t".encode()).digest()).decode()

        body = f"{cipher}|{user_id}|{daily_mini_game_name}|{score_cipher}|{sig}"
