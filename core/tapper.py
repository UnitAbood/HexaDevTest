        ...
        while True:
            try:
                ...
                if time() - access_token_created_time >= 3600:
                    ...
                    daily_mini_games = game_config.get('dailyKeysMiniGames', [])
                    for daily_mini_game_name in daily_mini_games:
                        daily_mini_game = daily_mini_games.get(daily_mini_game_name)
                        if daily_mini_game and settings.APPLY_DAILY_MINI_GAME:
                            is_claimed = daily_mini_game['isClaimed']

                            if is_claimed:
                                logger.info(f"{self.session_name} | Daily Mini Game <lc>{daily_mini_game_name}</lc> already claimed today")

                            while is_claimed == False:
                                seconds_to_next_attempt = -1 if daily_mini_game_name == "Tiles" else daily_mini_game['remainSecondsToNextAttempt']
                                ...
                                if not is_claimed and seconds_to_next_attempt <= 0:
                                    ...
                                    encoded_body = await get_mini_game_cipher(
                                        ...
                                        daily_mini_game_name=daily_mini_game_name
                                    )

                                    if encoded_body:
                                        await start_daily_mini_game(http_client=http_client,
                                                                    miniGameId=daily_mini_game_name)
                                        ...
                                        if daily_mini_game:
                                            ...
                                            if is_claimed:
                                                ...
                                                logger.success(f"{self.session_name} | Successfully claimed Mini Game <lc>{daily_mini_game_name}</lc> | "
                                                               f"Total keys: <le>{total_keys}</le> (<lg>+{calc_keys}</lg>)")

                                else:
                                    if seconds_to_next_attempt > 0:
                                        logger.info(f"{self.session_name} | "
                                                    f"Need <lw>{seconds_to_next_attempt}s</lw> to next attempt in Mini Game <lc>{daily_mini_game_name}</lc>")
                                    elif not encoded_body:
                                        logger.info(f"{self.session_name} | Key for Mini Game <lc>{daily_mini_game_name}</lc> is not found")
                ...
                if settings.AUTO_UPGRADE is False and self.session_name != "Leha":
